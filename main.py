...
from queries.core import async_create_tables, SynchCore, AsynchCore, Test
...
async def main():
    ...
    await AsynchCore.insert_data_orm("users",username=Misha.username, level=Misha.level, rating=Misha.rating, experience=Misha.experience)
    await AsynchCore.insert_data_orm("users",username=Vitya.username, level=Vitya.level, rating=Vitya.rating, experience=Vitya.experience)
    await AsynchCore.insert_data_orm("users",username=Ifmi.username, level=Ifmi.level, rating=Ifmi.rating, experience=Ifmi.experience)
    await AsynchCore.insert_data_orm("users",username=artyom.username, level=artyom.level, rating=artyom.rating, experience=artyom.experience)
    await AsynchCore.insert_data_orm("tournaments",match_id=tournament_1.match_id,stage=tournament_1.stage, user1_id=tournament_1.user1_id,user2_id=tournament_1.user2_id)
